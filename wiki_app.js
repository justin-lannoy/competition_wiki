...
function CompetitorView({ navigateTo }) {
  // Recency emphasis: weight the trailing 90 days for what's surfaced.
  const cutoff90 = useMemo(() => new Date(Date.now() - 90 * 864e5).toISOString().slice(0, 10), []);
  const cards = useMemo(() => COMPETITOR_PAGES.map(c => {
    const allMoves = movesForCompetitor(c.slug);
    ...
    const recent90 = allMoves.filter(e => (e.date || '') >= cutoff90).length
                   + recentNews.filter(n => (n.date || '') >= cutoff90).length;
    return {
      c, moves, recentNews, tracker, filings: tracker ? (parseInt(tracker.count, 10) || 0) : 0,
      news, newsCount: news ? (parseInt(news.count, 10) || 0) : 0, recent90,
    };
  }), [cutoff90]);
  ...
  return (
    <div className="main-content">
      ...
      <div className="md-content">
        ...
        {TIER_ORDER.map(tier => {
          // Most-active-in-90-days first within each tier.
          const group = cards.filter(x => (x.c.tier || 'other') === tier)
            .sort((a, b) => b.recent90 - a.recent90);
          if (!group.length) return null;
          const tier90 = group.reduce((n, x) => n + x.recent90, 0);
          return (
          <div key={tier} className="tier-group">
            <h2 className="tier-heading">{TIER_LABELS[tier] || tier}<span style={{ color: 'var(--muted)', fontWeight: 400, fontSize: 13, marginLeft: 8 }}>{group.length} · {tier90} in 90d</span></h2>
            {group.map(({ c, moves, recentNews, tracker, filings, news, newsCount, recent90 }) => (
